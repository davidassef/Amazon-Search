...
export class UI {
  ...
  renderComparisonView(data) {
    ...
    // Create a column for each domain
    domains.forEach(domain => {
        tableHTML += `<div>`;
        tableHTML += `<h2 class="font-bold text-lg text-center mb-4">${domain}</h2>`;
        const products = data.results[domain].products;
        if (products.length > 0) {
            products.forEach(product => {
                tableHTML += this.renderProductCard(product);
            });
        } else {
            tableHTML += `<div class="text-center text-gray-500">${this.i18n.t('noProducts')}</div>`;
        }
        tableHTML += `</div>`;
    });
